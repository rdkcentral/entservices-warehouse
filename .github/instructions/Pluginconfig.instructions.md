...
- Each plugin must define <PluginName>.conf.in file that includes the following mandatory properties:
  ...
  - **callsign**: A unique identifier used to reference the plugin within the framework. Every callsign must be defined with a prefix of org.rdk followed by the ENT Service name written in PascalCase (e.g., org.rdk.PersistentStore).
